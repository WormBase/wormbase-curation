...
#             if ($label) { $trData .= qq(<td>$label</td><td>$i</td><td>$inputtermidvalue</td>\n); }
  ...
  if ($submit_flag eq 'submit') { 
      ...
      if ($mandatoryFail) { 
          ...
          $form_data = qq(Dear $fields{person}{field}{person}{inputvalue}{1}, thank you for submitting Micropublication data. Your data will be sent to 2 referees for external review. We will contact you after the evaluation is completed. If you have additional questions, please contact <a target="new" href="http://www.wormbase.org/tools/support">WormBase</a>.<br/><br/>) . $form_data;			# prepend thank you message
...
#   print qq(Bookmark this <a href="$saveUrl">link</a> to load it in the future.<br/>\n);
  ...
  if ($fields{email}{field}{email}{inputvalue}{1}) {
    ...
    my $subject = 'Link for WormBook Micropublication form';
    my $gene    = 'no gene chosen';     
    if ($fields{gene}{field}{gene}{inputvalue}{1}) { $gene = $fields{gene}{field}{gene}{inputvalue}{1}; }
    my $date    = &getPgDate();
    my $body    = qq(Please continue your submission through the WormBook Micropublication form, for $gene, saved on $date, with this link: $saveUrl\n\nThank you\nWormBook/WormBase team);
